...
{{-- Page Title --}}
@section('title')
    {{ config('app.name') }} » {{ trans('cortex/foundation::common.tenantarea') }} » {{ trans('cortex/contacts::common.contacts') }} » {{ $contact->exists ? $contact->name : trans('cortex/contacts::common.create_contact') }}
@stop

@push('scripts')
    {!! JsValidator::formRequest(Cortex\Contacts\Http\Requests\Tenantarea\ContactFormRequest::class)->selector('#tenantarea-contacts-save') !!}
        ...
        <section class="content">
            ...
            <div class="nav-tabs-custom">
                <ul class="nav nav-tabs">
                    ...
                    @if($contact->exists) <li><a href="{{ route('tenantarea.contacts.logs', ['contact' => $contact]) }}">{{ trans('cortex/contacts::common.logs') }}</a></li> @endif
                    @if($contact->exists && $currentUser->can('delete-contacts', $contact)) <li class="pull-right"><a href="#" data-toggle="modal" data-target="#delete-confirmation" data-item-href="{{ route('tenantarea.contacts.delete', ['contact' => $contact]) }}" data-item-name="{{ str_slug($contact->name) }}"><i class="fa fa-trash text-danger"></i></a></li> @endif
                </ul>
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane active" id="details-tab">
                        ...
                        @if ($contact->exists)
                            {{ Form::model($contact, ['url' => route('tenantarea.contacts.update', ['contact' => $contact]), 'method' => 'put', 'id' => 'tenantarea-contacts-save']) }}
                        @else
                            {{ Form::model($contact, ['url' => route('tenantarea.contacts.store'), 'id' => 'tenantarea-contacts-save']) }}
                        @endif
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    @include('cortex/foundation::tenantarea.partials.timestamps', ['model' => $contact])
