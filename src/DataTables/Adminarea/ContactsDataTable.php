...
class ContactsDataTable extends AbstractDataTable
{
    ...
    protected function getColumns(): array
    {
        $link = config('cortex.foundation.route.locale_prefix')
            ? '"<a href=\""+routes.route(\'adminarea.cortex.contacts.contacts.edit\', {contact: full.id, locale: \''.$this->request()->segment(1).'\'})+"\">"+data+"</a>"'
            : '"<a href=\""+routes.route(\'adminarea.cortex.contacts.contacts.edit\', {contact: full.id})+"\">"+data+"</a>"';
