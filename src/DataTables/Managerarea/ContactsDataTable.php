...
class ContactsDataTable extends AbstractDataTable
{
    ...
    protected function getColumns(): array
    {
        $link = config('cortex.foundation.route.locale_prefix')
            ? '"<a href=\""+routes.route(\'managerarea.contacts.edit\', {contact: hashids.encode(full.id), locale: \''.$this->request->segment(1).'\'})+"\">"+data+"</a>"'
            : '"<a href=\""+routes.route(\'managerarea.contacts.edit\', {contact: hashids.encode(full.id)})+"\">"+data+"</a>"';
